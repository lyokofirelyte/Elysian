...
public class SpleefCommandMain {
	...
	public void onSpleef(Player p, String[] args){
		...
		switch (args[0].toLowerCase()){
			...
			case "invite":
				...
				if (a(sender, args.length, 2)){
					if (main.main.doesPartialPlayerExist(args[1])){
						SpleefPlayer them = null;
						them = doesPlayerExist(args[1]) ? matchSpleefPlayer(args[1]) : new SpleefStorage(main.main, SpleefDataType.PLAYER, main.main.getPlayer(args[1]).getName()).toPlayer();
			...
			case "score":
				...
				if (a(sender, args.length, 2)){
					if (main.main.doesPartialPlayerExist(args[1])){
						try {
							sender.toDp().s("Total Score: " + main.main.matchDivPlayer(args[1]).getInt(SpleefPlayerData.TOTAL_SCORE));
							sender.toDp().s("Game Wins: " + main.main.matchDivPlayer(args[1]).getInt(SpleefPlayerData.TOTAL_WINS));
							sender.toDp().s("Game Losses: " + main.main.matchDivPlayer(args[1]).getInt(SpleefPlayerData.TOTAL_LOSSES));
						} catch (Exception e){
	...
	private boolean perms(Player p, String perm){
		return main.main.silentPerms(p, perm);
	}
}