...
public class TeamPVP extends THashMap<String, TeamPVPGame> implements DivGame, ElySave {
		...
		if (!containsKey(name)){
			...
			put(name, new TeamPVPGame(){
				...
				public void finish(String[] winners){
					...
					DivinityUtils.bc("&6" + winners[0] + " &band &6" + winners[1] + " &bhave won a TeamPVP match!");
					
					for (TeamPVPPlayer p : players){
						if (main.isOnline(p.name())){
							main.getPlayer(p.name()).getInventory().clear();
						}
					}
					...
					if (!lobby){
						...
						for (TeamPVPPlayer p : t1){
							if (main.isOnline(p.name())){
								...
								main.api.event(new DivinityTeleportEvent(main.getPlayer(p.name()), newLoc));
							}
						}
						...
						for (TeamPVPPlayer p : players){
							if (main.isOnline(p.name())){
								main.api.event(new DivinityTeleportEvent(main.getPlayer(p.name()), lobbyLoc));
							}
						}
					}
				}
	...
	public TeamPVPPlayer createPlayer(final String name){
		...
		return new TeamPVPPlayer(){
			...
			public DivinityPlayer toDp(){
				return main.matchDivPlayer(name);
			}
