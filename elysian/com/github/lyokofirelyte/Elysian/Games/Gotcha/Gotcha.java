...
public class Gotcha extends THashMap<String, GotchaGame> implements DivGame, ElySave {
		...
		if (!containsKey(name)){
			...
			put(name, new GotchaGame(){
				...
				public void stop(){
					...
					DivinityUtils.bc(winner + " &bhas won a game of Gotcha!");	
					remove(name);
				}
				...
				public void actuallyStart(){
					...
					if (players.size() > 0){
						...
						main.api.activeTasks.put("gotchaCounter" + name, Bukkit.getScheduler().scheduleSyncRepeatingTask(main, new Runnable(){ public void run(){
