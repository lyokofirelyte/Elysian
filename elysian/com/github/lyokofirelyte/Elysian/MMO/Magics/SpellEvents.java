...
public class SpellEvents extends ElyMMO implements Listener {
	...
	public void onBow(EntityShootBowEvent e){
		if (e.getProjectile() instanceof Arrow){
			...
			main.api.repeat(main.mmo.spellTasks, "normalArrow", 0L, 1L, main.spellTasks.get(pro), main, pro);
		}
	}
		...
		if (e.getAction() == Action.RIGHT_CLICK_AIR){
			...
			if (p.getItemInHand() != null && p.getItemInHand().hasItemMeta() && p.getItemInHand().getItemMeta().hasDisplayName() && p.getItemInHand().getItemMeta().hasLore()){
				if (Spell.FIRE_BLAST.contains(ChatColor.stripColor(main.AS(p.getItemInHand().getItemMeta().getDisplayName())))){
					try {
						Spell.valueOf(ChatColor.stripColor(main.AS(p.getItemInHand().getItemMeta().getDisplayName().toUpperCase().replace(" ", "_")))).cast(main, p);
					} catch (Exception ex){
