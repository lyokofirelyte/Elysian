...
public class ElyPatrol {
	...
	public void sendMessage(Patrol patrol, JSONChatMessage message){
		for (String player : patrol.getMembers()){
			message.sendToPlayer(main.getPlayer(player));
		}
	}
	
	public void sendMessage(Patrol patrol, String message){
		for (String player : patrol.getMembers()){
			main.s(main.getPlayer(player), message);
		}
	}
	...
	public void onPatrolChat(Player p, String[] args){
		...
		if (doesPatrolExistWithPlayer(p)){
			...
			sendMessage(getPatrolWithPlayer(p), main.api.json.create("&3P &7\u2744 ", ImmutableMap.of(
				...
				p.getDisplayName() + "&f: &3" + main.api.divUtils.createString(args, 0), ImmutableMap.of(
		...
		if (args.length > 0){
			...
			switch (args[0]){
				...
				case "#add":
					...
					DivinityUtils.bc("Patrol formed by " + p.getDisplayName() + " &6(&3" + args[1] + "&6)&b!");
					...
					if (dp.getLevel(ElySkill.PATROL) >= 20){
						if (System.currentTimeMillis() >= dp.getLong(DPI.PATROL_TP_COOLDOWN)){
							if (main.isOnline(args[1])){
								...
								main.api.event(new DivinityTeleportEvent(p, main.getPlayer(args[1]).getLocation()));
								sendMessage(getPatrolWithPlayer(p), "&6System&f: &bPatrol TP " + p.getDisplayName() + " &b-->> " + main.getPlayer(args[1]).getDisplayName());
							} else {
			...
			main.s(p, main.api.json.create("&3Elysian Patrol System ", ImmutableMap.of(
				"&a{+}", ImmutableMap.of(
					JSONClickType.CLICK_RUN, new String[]{
						"/patrol #createInput",
			...
			for (Patrol patrol : getAllPatrols()){
				...
				main.s(p, main.api.json.create("&6" + patrol.name() + " ", ImmutableMap.of(
